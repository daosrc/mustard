...
| **离线词典** | 本地 → 在线 → AI 分级查询；词典数据**不打包**，**首次使用时按需下载**（默认英汉《现代英汉》，另有 ECDICT MIT 示例、Wordset 英英按字母下载），下载后可离线查词 |
| **模型提供商** | 默认 OpenRouter（OpenAI 兼容，含免费模型），可自定义提供商，模型逐条添加并标注输入能力 |
...
1. **配置模型（AI 功能）**：打开扩展设置，填入 OpenRouter（或其他提供商）的 API Key，或添加自定义提供商。
