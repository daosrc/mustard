...
- CSS 变量：`--m-paper / surface / surface-2 / ink / muted / faint / line / primary / primary-ink / primary-soft / accent / accent-soft / blush / warn`
- UnoCSS：`presetMustard()`（颜色映射到上述变量）；`theme.css`（浅色 + `[data-theme="dark"]`）
...
```
然后阅读：`AGENTS.md` → `design/PLAN.md` → `docs/HANDOFF.md`（本节）。下一个里程碑：**M3 · UI 组件库 + 主题**。
